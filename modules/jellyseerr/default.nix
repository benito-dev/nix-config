...
in {
  options.services.jellyseerr.init.enable =
    lib.mkEnableOption "Initializing Jellyseerr";
      ...
      serviceConfig = {
        ...
        ${pkgs.curl}/bin/curl -s -X 'POST' "http://localhost:${port}/api/v1/settings/sonarr/" \
          ...
          -d '{
            ...
            "activeAnimeProfileName":"Main",
            "activeAnimeDirectory":"'"${config.services.sonarr.init.rootPath}"'",
            "tags":[],
            "animeTags":[],
            "is4k":false,
            "isDefault":true,
